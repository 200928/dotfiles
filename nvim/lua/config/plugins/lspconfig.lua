  ...
  },
  {
    ...
    dependencies = {
      ...
      },
      {
        ...
        --       local buf = args.buf
        --       vim.api.nvim_buf_set_keymay(buf, "n", "<C-M-l>", "<cmd>lua vim.lsp.buf.format()<CR>", { noremap = true })
        --     end,
        --   })
        -- end
        -- opts = {
    ...
    config = function()
      ...
      -- LspAttach 的时候，设置对应 buff 的格式化 keymap
      vim.api.nvim_create_autocmd("LspAttach", {
        callback = function(args)
          local buf = args.buf
...
F.configureDocAndSignature = function()
  vim.lsp.handlers["textDocument/signrequire('lsp-inlayhints').toggle()atureHelp"] = vim.lsp.with(
